  ...
  switch (disasterAutoType.value){
    case 1:{
      ...
      d.on("update",(time,frameIndex,frameLength)=>{
        ...
        if(frameIndex === 39){
          createRootDisaster(
              new THREE.Vector3(15259.268718840352,1312.0609130859377,-4397.882820623991),
              new THREE.Vector3(15582.487762907616,1312.0609130859375,-4380.362858235656),
              1
          )
        }
    ...
    case 3:{
      ...
      },expandingDisaster,disasterAutoType.value,2000,1000*28);
      d.on("end",()=>{
        playing.value = false;
    ...
    case 5:{
      let d = new Drama(()=>{
        ...
        doEntranceAnimationForWater(waterMessage.resultObject,35,1000*33);

      },expandingDisaster,disasterAutoType.value,1000,1000*66);
      d.on("end",()=>{
        playing.value = false;
        ...
        setTimeout(()=>{
          doEntranceAnimationForWater(waterMessage.resultObject,50,2000);
        },3000)
      });
      d.on("update",(time,frameIndex,frameLength)=>{
        console.log(time,frameIndex,frameLength);
...
function initYUKA(){
  ...
  loader.load(base+'/0_9.glb').then((navigationMesh ) => {
...
onMounted( async () => {
  ...
  //初始化相机
  camera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.1, 9999999);
  camera.position.z = 5;
